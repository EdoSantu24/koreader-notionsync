...
All progress and cancellation goes through `NotionSync:tick(text, force)`. It is throttled by *time* (~1s), so an unchanged page causes no repaint while a slow page still shows movement; cancellation latency is therefore bounded by the throttle interval plus the in-flight network timeout. Cancellation is checked between databases, between pages, inside `blocktree` (via `should_abort`, because one page can make dozens of requests), before each image download, and **between the slices of a retry backoff**.

That last one is why `api.lua` has a `should_abort` field and a `waitFor` rather than calling `socket.sleep` directly. A single `socket.sleep(delay)` blocks KOReader's event loop for the whole delay, so nothing repaints *and* `Trapper` never sees the dismiss tap — on e-ink, indistinguishable from a hang. `waitFor` sleeps in `SLEEP_SLICE` (0.25s) chunks and consults the hook between them, returning `false` if the wait was cut short so `apiCall` abandons the retry rather than merely stopping the wait.

Two things about that hook are load-bearing. `main.lua` wires it to **`tick`, not to `sync_alive` directly**, because `Trapper:info` is simultaneously the repaint and the dismiss check — a hook reading only the flag would leave the sync interruptible in principle while never noticing the interruption. And it is cleared in `runSync`'s teardown, not at the end of `runSyncLoop`: the teardown is the path that runs even when the loop throws, and a stale hook would see `sync_alive` false from the finished sync and abort the *next* request the plugin made — the database picker, with no sync running at all. `waitFor` treats a missing hook as "just sleep", which is what those non-sync paths rely on.
